    ...
    for (const c of this.cena.colisores) {
      ...
      if (d2 < this.raioAviao * this.raioAviao) {
        ...
        if (this.predioAvisado !== c.participante) {
          ...
          this.mostrarAviso(`💥 Bateu no prédio de <strong>${escaparHtml(c.participante.apelido)}</strong> — <strong>${escaparHtml(formatarHoras(c.participante.horas, c.participante.horasTexto))}</strong> Blackboard`, 2800, true);
